function obj = computeMSDparfor(obj, indices)
...
for i = 1 : n_tracks
    ...
    % Sliced output variables for parfor loop
    all_index   = cell(n_detections - 1, 1);
    dr2s        = cell(n_detections-1, 1);
...
%                 index_in_all_delays = 1 + ceil(obj.logbase.^n);
            ...
            otherwise
                disp(['You chose method ' sampling ' which is not supported in computeMSDparfor'])
                error('Will is a lazy bugger, sorry!')
        end
        
        % Square displacement in bulk
        dX = X(j+1:end,:) - repmat(X(j,:), [(n_detections-j) 1] ); %#ok<PFBNS>
        dr2 = sum( dX .* dX, 2); % Each dimension has 1 column
        
        % Store for mean computation / Knuth
        all_index{j} = index_in_all_delays;
        dr2s{j}(index_in_all_delays) = dr2;
    end
    
    % Mean computation / Knuth (whatever that means)
    for j = 1:n_detections - 1
        n_msd(all_index{j}) = n_msd(all_index{j}) + 1;
        delta = dr2s{j} - mean_msd(all_index{j});
        mean_msd(all_index{j}) = mean_msd(all_index{j}) + delta ./ n_msd(all_index{j});
        M2_msd2(all_index{j})  = M2_msd2(all_index{j}) + delta .^2;
    end
