...
%% Example 3: Double exponential relaxation (extreme amplitude version)
...
%% Example 4: Double exponential relaxation (realistic amplitude version)
...
% at time 0).
tau = 1;    % Time (in seconds) to turn 'on' field
A0 = 2e3;   % Total amplitude (i.e. value at t=tau). This value seems about right
A1 = 0.93*A0;   % Amplitude of component 1
T1 = 6.4e-1;  % Relaxation time of first component
T2 = 0.36;    % Relaxation time of second component
% Then we declare the function by starting with an '@' and the input
% arguments in parenthesis. 
E_func = @(x, y, z, t) [1, 1, 0] .* (reshape(t - tau,1,1,[]) >= 0) .* (A1 * exp(-reshape(t - tau,1,1,[]) ./ T1) ...
    + (A0 - A1) * exp(-reshape(t - tau,1,1,[]) ./ T2));
