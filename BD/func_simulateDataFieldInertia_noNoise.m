function [varargout] = func_simulateDataFieldInertia_noNoise(opts)
%% [tracks, Xforces, stiffnesses, msdanalyzer] = func_simulateDataFieldInertia_noNoise(opts)
%% Motion of particle in optical trap and electric field, inertial regime, no noise
...
% No random Gaussian noise
if opts.rng_seed ~= 0; rng(opts.rng_seed); end
noise = zeros(Nb, 3, N);% .* sqrt(2 * kB * T .* gamma0 / deltat) ;
